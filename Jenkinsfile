pipeline {
  ...
  environment {
    IMAGE = "vinu890/flask-ci-cd"   // <-- Replace with your Docker Hub username/repo
  }
...
for i in $(seq 1 12); do
  ...
  if docker run --rm --network bluegreen_net curlimages/curl:latest -sS "http://$NEW_NAME:80/health" >/dev/null 2>&1; then
    OK=1
    break
  fi
  sleep 5
...
# nginx config to proxy to new container
...
http {
  server {
    listen 80;
    location / {
      proxy_pass http://__UPSTREAM__:80;
